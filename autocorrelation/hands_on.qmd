...
scatter plots and spot outlier observations, it is common practice to standardise the
...
The outcome of the spatial autocorrelation assessment is heavily dependent on the spatial
...
to variogram you may know, but shows the change of the autocorrelation index depending
on the change of the extent used to generate distance-based (KNN or band) weights.
...
default, `esda.correlogram` uses Moran's $I$, with the index being accessible as `correlogram.I`.
...
The correlogram can be used to show Moran's $I$ or any other autocorrelation statistic,
