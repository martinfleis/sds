...
In the last section, you learnt how to encode spatial relationships between geometries
...
We can calculate the spatial lag for the variable `"Petr Pavel"` and store it directly
...
```{python}
neis = elections.loc[contiguity_r['Abertamy'].index, "Petr Pavel"]
neis
```
...
3. And the spatial weights matric. Since `contiguity` is using the new `Graph` and `esda`
hasn't been updated yet, you need to convert it to the older `W` object.
...
PySAL's 'esda' module directly:

```{python}
mi = esda.Moran(elections['Petr Pavel'], contiguity_r.to_W())  # <1>
```
1. `Moran` assumes a row-standardised matrix in the old `W` format. Note how you do not need to use the standardised version of the variable in this context, as you will not represent it visually.
...
```{python}
elections.loc[lisa.p_sim < 0.05, 'cluster'] = lisa.q[lisa.p_sim < 0.05]     # <1>
elections["cluster"] = elections["cluster"].fillna(0)                       # <2>
elections["cluster"] = elections["cluster"].map(                            # <3>
    {                                                                       # <3>
        0: "Not significant",                                               # <3>
        1: "High-high",                                                     # <3>
        2: "Low-high",                                                      # <3>
...
elections.head()
```
1. Use `lisa.p_sim < 0.05` as a filter to assign a quadrant from `lisa.q` only to observations that can be
considered significant.
2. Fill the remaining - non-significant - observations with 0.
3. Replace the numerical codes of quadrants with spelled-out versions.
...
global Moran's $I$, you select 5% as the threshold for statistical significance. To
identify these values, you use `lisa.p_sim < 0.05` to create a mask, that contains
`True` if the $p$-value of the observation satisfies the condition, and `False` if
otherwise. Then, you can assign the actual quadrant and drop that are not significant.
...
presented, you can "cook" the plot ourselves:
...
Try `elections.explore("cluster", prefer_canvas=True)` to create an interactive map of
LISA cluster for easier exploration.
:::
...
spatial structure of the vote emerge in a more explicit way. The result of this
...
`splot` also comes with a way of plotting the Moran Plot, allowing you to link both
together:
