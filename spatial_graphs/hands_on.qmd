...
Spatial lag is usually used to derive the mean value from neighbors. However, if you
are interested in a more robust description of the distribution, you can use the
`summary()` method.
...
Notice that `mean` column matches the spatial lag computed using the `lag()` method, as
expected.
...
columns = ["area", "area_lag", "std", "max"]  # <1>
