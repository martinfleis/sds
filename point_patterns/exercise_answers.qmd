...
```py
mean_center = pointpats.centrography.mean_center(airbnb)
med_center = pointpats.centrography.euclidean_median(airbnb)
weighted_mean = pointpats.centrography.weighted_mean_center(
    airbnb, airbnb["calculated_host_listings_count"]
)
```
...
ax.scatter(*mean_center.xy, color="k", marker="o", label="Mean Center", alpha=0.8)
ax.scatter(*med_center.xy, color="r", marker="o", label="Median Center", alpha=0.8)
ax.scatter(
    *weighted_mean.xy,
    color="pink",
    marker="o",
...
```py
qstat = pointpats.QStatistic(airbnb, nx=6, ny=6)
```
...
```py
qstat = pointpats.QStatistic(airbnb, nx=12, ny=10)
```
