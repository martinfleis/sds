...
```{python}
ordinary_kriging = pyinterpolate.ordinary_kriging(
    theoretical_model=semivar,  # <1>
    known_locations=two_bed_homes[['geometry', 'price_lag']],    # <2>
    unknown_location=grid_10.centroid.iloc[0], # <3>
    no_neighbors=10,    # <5>
)
```
...
The resulting `ordinary_kriging` is a tuple with four values representing predicted value, variance error, x, and y.
You can select the first one and assign it as a column.
<!-- TODO: this is now broken as above assumes scalar location now for some reason -->
```{python}
