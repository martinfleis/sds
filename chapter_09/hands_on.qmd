...
```{python}
simd = gpd.read_file(
    # "https://martinfleischmann.net/sds/chapter_09/
...
[this link](https://martinfleischmann.net/sds/chapter_09/data/edinburgh_simd_2020.gpkg)
and saving the file
...
[Is there a pattern?](../chapter_06/exercise.qmd) exercise. The dataset for Edinburgh
...
airbnb = gpd.read_file(
    # "https://martinfleischmann.net/sds/chapter_09/"
    "data/edinburgh_airbnb_2023.gpkg"
)
airbnb.head()
```
...
1. Download the file by right-clicking on
[this link](https://martinfleischmann.net/sds/chapter_09/data/edinburgh_airbnb_2023.gpkg)
and saving the file
2. Place the file in the same folder as the notebook where you intend to read it
3. Replace the code in the cell above with:
...
[Clustering and regionalisation](../chapter_07/hands_on.qmd) section.
...
The result can be plotted and explored using the `plot()` method.
...
You can see from the plot of variance error, that anything further away from existing points is becoming fairly unreliable. You can, for example, set the specific threshold of the variance error you think is acceptable and treat all the other locations as _missing_ or _unreliable_.
